...
from theory_model_stock_gambling.model_functions import (
    calculate_equilibrium_solution,
    calculate_expected_utility,
)


def task_calculate_equilibrium_result(produces= BLD / "equilibrium_result.csv"):

    result = calculate_equilibrium_solution(
    ...
    result["Utility_Agent_1"] = calculate_expected_utility(W_0 = MODEL_RUN_CONFIGURATION["Initial_Wealth_Agent_1"], x = result["x_1"], prob_e_high = MODEL_RUN_CONFIGURATION["Endowment_Probability_High_Agent_1"],
    ...
    result["Utility_Agent_2"] = calculate_expected_utility(W_0 = MODEL_RUN_CONFIGURATION["Intial_Wealth_Agent_2"],
...
def task_calculate_equilibrium_result_sensitivity_riskiness_endowment_agent_2(produces= BLD / "equilibrium_result_sensitivity.csv"):
    ...
    for i in range(len(endowment_high_payoff_range)):
        ...
        welfare_agent_1[i] = calculate_expected_utility(W_0 = MODEL_RUN_CONFIGURATION["Initial_Wealth_Agent_1"], x = result["x_1"], prob_e_high = MODEL_RUN_CONFIGURATION["Endowment_Probability_High_Agent_1"],
        Return_e_high = MODEL_RUN_CONFIGURATION["Endowment_Payoff_High_Agent_1"], Return_e_low = MODEL_RUN_CONFIGURATION["Endowment_Payoff_Low_Agent_1"], prob_R_high = MODEL_RUN_CONFIGURATION["Stock_Probability_High"], Return_R_high = MODEL_RUN_CONFIGURATION["Stock_Payoff_High"],
        Return_R_low = MODEL_RUN_CONFIGURATION["Stock_Payoff_Low"],
        price = result["p"])

        welfare_agent_2[i] = calculate_expected_utility(W_0 = MODEL_RUN_CONFIGURATION["Intial_Wealth_Agent_2"],
        x = result["x_2"], prob_e_high = MODEL_RUN_CONFIGURATION["Endowment_Probability_High_Agent_2"],
